...
export async function cmdBatch(filePath: string) {
  ...
  try {
    ...
    if (!Array.isArray(tasks)) throw new Error("expected a JSON array")
  } catch (e: any) {
    console.log()
  ...
  const results: { label: string; success: boolean; hintsLoaded: number; hintsAdded: number }[] = []
