...
export function cmdStats(all = false) {
    ...
    console.log();
    console.log(chalk.dim("  " + "domain".padEnd(28)) +
    ...
    for (const store of stores) {
        ...
        const bar = (pct >= 80 ? chalk.green : pct >= 50 ? chalk.yellow : chalk.red)("█".repeat(filled)) + chalk.dim("░".repeat(10 - filled));
        const pctStr = (pct >= 80 ? chalk.green : pct >= 50 ? chalk.yellow : chalk.red)(`${pct}%`);
        console.log(`  ${chalk.bold(store.domain.padEnd(28))}` +
    ...
    if (all) {
        ...
        console.log(chalk.dim("  " + "─".repeat(64)));
        console.log(chalk.dim(`  total runs: ${totalRuns}  ·  total hints: ${totalHints}  ·  avg success: `) +
